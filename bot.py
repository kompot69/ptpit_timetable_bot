...
def worker(event):
    ...
    if pattern.startswith("бот группа "):
        ...
        logFile.close()
        text="Установка группы "+gr0up+"..."
        newmsg = api.messages.send(peer_id=message['peer_id'],random_id=0,message=text)
        #поиск
        ...
        if not gr0up in groups_format:
            gr0up=gr0up+"п"
            if not gr0up in groups_format:
                return api.messages.edit(peer_id=message['peer_id'],message_id=newmsg,message=text)
        ###
        ...
        logFile.close()
        # вывод
        api.messages.edit(peer_id=message['peer_id'],message_id=newmsg,message=text)
        #логгирование
        dateNow=str((datetime.now()+td(hours=5)).strftime("%d-%m-%y %H:%M:%S"))
        logFile=open("botLog.log", "a")
...
# # # # #
    ...
    if pattern.startswith("расписание на "):
        ...
        newmsg = api.messages.send(peer_id=message['peer_id'],random_id=0,message='Поиск расписания на '+mess_date+'...')
        # логгирование
        dateNow=str((datetime.now()+td(hours=5)).strftime("%d-%m-%y %H:%M:%S"))
        logFile=open("botLog.log", "a")
        ...
        if text != "":
            return api.messages.edit(peer_id=message['peer_id'],message_id=newmsg,message=f"Расписание на {dateneed}\n\n{text}")
        else:
            return api.messages.edit(peer_id=message['peer_id'],message_id=newmsg,message=f"Расписание на {dateneed} не найдено")
...
# # # # #
    ...
    if "пар" in pattern  and ("сейчас" in pattern or "следующ" in pattern):
        newmsg = api.messages.send(peer_id=message['peer_id'], random_id=0, message='Поиск...')
        # логгирование
        dateNow=str((datetime.now()+td(hours=5)).strftime("%d-%m-%y %H:%M:%S"))
        logFile=open("botLog.log", "a")
        ...
        # вывод
        if text != "":
            return api.messages.edit(peer_id=message['peer_id'],message_id=newmsg,message=title+text)
        else:
            return api.messages.edit(peer_id=message['peer_id'],message_id=newmsg,message=f"Пар на сегодня не найдено")
...
# # # # #
    ...
    if "статус" in pattern and "бот" in pattern:
        newmsg = api.messages.send(peer_id=message['peer_id'], random_id=0, message='Сбор данных...')
        text=" "
         #логгирование
        dateNow=str((datetime.now()+td(hours=5)).strftime("%d-%m-%y %H:%M:%S"))
        logFile=open("botLog.log", "a")
        ...
        # вывод
        return api.messages.edit(peer_id=message['peer_id'],message_id=newmsg,message=text,dont_parse_links=1)
