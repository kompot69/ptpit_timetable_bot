# ptpit-timetable vk bot controller
version='0.4.1 vk'
import os
...
checkGroupIDsFile=os.path.isfile('groupID.py')

if checkGroupIDsFile == False:
    dateNow=str((datetime.now()+timedelta(hours=5)).strftime("%d-%m-%y %H:%M:%S"))
    print(dateNow+"] [BC] start without groupID file, creating")
    # input groupID
    groupID=input("[BC] Input your group ID (only numbers): ")
    # create groupID
    File=open("groupID.py", "a")
    File.write("owner_id=-"+groupID)
    File.close()
