...
def getTimetable(dateneed, gr0up, inline):
    if 1==1:
        ...
        if text != "":
            ...
            if inline:
                return "Расписание на "+dateneed+" для группы "+group+" не найдено."+checkNotice(datestrf)
...
def sendMessage(chatid, text, context):
    if int(chatid) < 0:
        ...
        if "е!" in text:
            keyboard=InlineKeyboardMarkup([[InlineKeyboardButton(text='Открыть сайт', url='https://timetable.ptpit.ru')]])
        #message=context.bot.send_message(chat_id=chatid, text=text )#, reply_markup=keyboard) 
